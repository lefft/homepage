...
The use of twitter data for estimating public sentiment about issues can still be considered a fledgling-ish field. Here we assess the degree to which a fairly naive sentiment classifier can be used to predict the trajectory of the average bitcoin trading price, over time. The catch is that we'll only use data publicly available via the twitter search API. This is limiting in a few respects, chief among which is that targeting specific historical date ranges is basically impossible with only the standard search tools (and without third-parties). A scaled-up (and idealized) version of this strategy might use historical firehose data to systematically extract bitcoin-related tweets at particular time intervals during which (or just before which) bitcoin trading prices are independently known to be volatile. 
...
```{r plot_ext_data_on_dat_interval}
...
```{r include=FALSE}
rm(tdat_simple); rm(bitcoinity); rm(bitcoinity_simple)
rm(merged); rm(merged_zoom); rm(price_diffs)
```
...
```{r cleanup_intermediate, include=FALSE}
rm(unabomber_manif); rm(unabomber_english_clf); rm(unabomber_multilg_clf)
rm(song_of_solomon); rm(songos_english_clf); rm(songos_multilg_clf)

rm(una_tbl_english); rm(una_tbl_multilg)
rm(sos_tbl_english); rm(sos_tbl_multilg)
```
...
# (note: we're just setting 'since', bc api won't play nice w small ranges)
...
```{r include=FALSE}
rm(tweets); rm(tweets_clf); rm(tweets_tbl); rm(tweets_confident)
```
...
### 3. btc-related tweet sentiment against btc price

<hr style="height:1px; background-color:lightgray; color:lightgray;">

<!-- DOESNT WORK CONSISTENTLY OVER DIFFERENT RUNS... -->
...
There's a number of ways we could approach the problem of associating sentiment about bitcoin to the price of bitcoin. The nature of the twitter search API limits our strategies severely.[^4] So here is the basic strategy we will use: 

- collapse the price data to the day-level to make things more manageable and to simplify the problem; 
- identify some key dates, when the bitcoin price increased or decreased "substantially;" 
- query several bitcoin-related different terms over the course of the price data date range; 
- tag each tweet according to whether its date is the day (ideally: hour) before one of the "substantial" price change days;
- filter out all the tweets that don't occur on such days (possibly iterating over previous two steps if this leaves us with too little data); 
- separate the tweets into two groups: 
    + `inc_tweets`: those occurring just before price *increases*, and 
    + `dec_tweets`: those occurring just before price *decreases*.
- Then, we use a monkeylearn classifier to measure the sentiment of each set of tweets; and finally, 
- compare the sentiment of `inc_tweets` to that of `dec_tweets`.

To make life easier, we'll start by collapsing the price data to the individual-day level.[^5] 
...
Below, we'll select the set of $n\times2$ dates on which the BTC trading price differed maximally from the previous day: $n$ where it increased the most; and $n$ where it decreased the most. Playing around with different values for $n$ (and different measures of "difference") will yield different perspectives on the data.

Let's start with $n=30$ just for kicks. 
...
#### 3.1 identify sharpest *increase* days 

Let's grab the `r Ndates` largest increase days:

```{r get_inc_days}
# get row indices for Ndates largest increase days
topN <- order(dat_day$price_change, decreasing=TRUE, na.last=TRUE)[1:Ndates]

# get the date vals for those days
(inc_days <- as.character(dat_day[topN, ]$date))
```

For now, we'll only use the dates but in principle the actual prices could be useful too. 

#### 3.2 identify sharpest *decrease* days 

Now get the `r Ndates` largest decrease days:

```{r get_dec_days}
# get row indices for Ndates largest decrease days
botN <- order(dat_day$price_change, decreasing=FALSE, na.last=TRUE)[1:Ndates]

# get the date vals for those three days
(dec_days <- as.character(dat_day[botN, ]$date))
```

```{r include=FALSE}
rm(Ndates); rm(topN); rm(botN)
```


#### 3.3 obtain relevant tweets over the date-range of the price data

<!-- Unfortunately but unsurprisingly, the increase days are basically adjacent to two of the decrease days. Even so, though, we can just request a shitload of tweets, toss the ones that aren't on the target date, and request again. If we do this enough times, we'll end up with a suitable number of tweets for each day. Let's set the threshold at 100.  -->

We'll now search for bitcoin-related tweets over the range of dates in our price data. We'd rather just get tweets for specific days of interest, but that's not feasible with the current querying setup (*trust me*)...Since we can't get exactly what we want, we'll just querying API a bunch of times over the date range we want, filtering out any tweets that don't come from one of the dates of interest in `inc_days` or `dec_days`. **update**: this strategy fails miserably because the search API really only gives you reasonable amounts of data (if any) from the last seven days (see this [stack overflow question](https://stackoverflow.com/questions/39992266/error-n-tweets-were-requested-but-the-api-can-only-return-0), for instance).

Eventually we'll have (or pretend to have) "enough" btc-related tweets that were tweeted on one of the days of interest. We'll then categorize them as belonging to the "increase" or "decrease" category, and then get sentiment scores for them from a monkeylearn classifier. After we get the sentiment classifications, we'll (informally + impressionistically + if I ever revisit this statistically) assess the relationship between increase/decrease of btc price and the sentiment of the temporally associated tweets. **note**[^6] 

[^6]: Querying specific date ranges doesn't return enough results...In other words, looping over dates and setting the `since` and `until` arguments of `searchTwitter()` as `since=as.character(as.Date(dates[x]) - 1)` and `until=dates[x]` will work sometimes but it isn't guaranteed to give you data for all dates. If you then just query over and over, you'll either just get the same tweets back over and over, or you'll get rate blocked by twitter!
...
# terms of interest
terms <- c("bitcoin", "btc", "#bitcoin", "#btc", "#cryptocurrency",
           "buy bitcoin", "sell bitcoin", "trade bitcoin")

# collect tweets for each month in the date range, and for each term
months <- paste0("2017-0", 1:6, "-01")

# container to catch query results
twitlist <- vector(mode="list", length=(length(months)-1) * length(terms))

# name each df for easier access
names(twitlist) <- unlist(lapply(months[1:5], function(x){
  paste(x, terms, sep="_")
}))
...
# m <- 3; x <- 5
# loop over months with -1 lag (see since/until)
for (m in 1:(length(months)-1)){
  
  # loop over the terms -- try to grab them all for each month
  for (x in seq_along(terms)){
    
    # make a counter for easier indexing with `twitlist` container
    cntr <- ((m - 1) * length(terms)) + x
    
    # for month m and term x, query x from day before m/01, until last day of m
    twitlist[[cntr]] <- searchTwitter(
      searchString=terms[x], n=100,
      since=as.character(as.Date(months[m])   -1), 
      until=as.character(as.Date(months[m+1]) -1)
    )
  }
}
...
pricechange_tweets <- 
  ...
  # mutate(from_query = names(twitlist)) %>% 
  filter(date %in% dates) %>% data.frame() 

# make sure to filter out all duplicates too 
pricechange_tweets <- 
  pricechange_tweets[which(!duplicated(pricechange_tweets$text)), ]

```


Now all of the tweets are in a single data frame, with the same structure as the original @bitcoinprice tweet data from which we extracted the price time-series. We'll add a column encoding whether the tweet occurred on a "decrease" day or an "increase" day, by referencing the `date` column of the price data.


```{r include=FALSE}
# rm(twitlist); rm(x)
# since=as.character(as.Date(min(dates)) - 1), 
# until=as.character(as.Date(max(dates)) - 10)
...
```{r}
# the dates we're targeting
kable(cbind(inc_days, dec_days))
...
```{r show_decdays_sentiment, eval=FALSE}

# NOT EVALUATED HERE...

if (nrow(dec_tweets_tbl) > 0){
...
# function to compute the sentiment ratio (num_pos over num_neg)
...
rm(inc_tweets); rm(dec_tweets); rm(inc_tweets_clf); rm(dec_tweets_clf); 
rm(inc_tweets_tbl); rm(dec_tweets_tbl)
```
...
Timing and time-resolution issues are really at the heart of the problem -- if something like this hypothetical bot is to be built, its effectiveness will be determined largely by the granularity of its BTC price measurements versus the granularity of its BTC tweet sentiment measurements. Put simply: collapsing data to the day-level is probably waaaay too coarse to be useful for actually anticipating price changes before they happen. But alas, let's just tell ourselves we're studying feasibility right now, so that we don't have to be bothered by such concerns (critical as they may be).
...
5. What's the best way to measure "sentiment"? How do we use random samples of btc-related tweets to establsish a kind of baseline, against which tweets from dates of interest can be evaluated. 


[^4]: shoot me [an email](mailto:tjleffel@gmail.com) if you want to hear about the other strategies that are way better conceptually but just don't work in practice (given the current tools I'm using).

[^5]: I think the mean over each hourly tweet should be fine; maybe median could give us a slightly different perspective. Not sure, but worth thinking about some more. 
