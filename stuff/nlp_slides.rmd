...
date: "timothy leffel // [http://lefft.xyz](http://lefft.xyz) // april05/2018"
output: 
...
<!-- 
  ## TIM NOTE, feb06/2019: 
    went back + fixed typos + made minor changes directly in .html file!!! the source .rmd does ***not*** reflect these changes! be aware if ever reusing parts of these slides... less than great but recentish changes to ioslides package make rebuild break + i can't spend the time investigating today 

  ## TIM NOTE, feb06/2019: 
    fixed date in header since m latt canceled + rescheduled the original mtg/pres date (was march28, morning of pres got moved to april05) 
...
but reference implementations are often written in lower-level languages -- e.g. both [`word2vec`](https://code.google.com/archive/p/word2vec/) and [`GloVe`](https://nlp.stanford.edu/projects/glove/) are written in C; [`Stanford CoreNLP`](https://stanfordnlp.github.io/CoreNLP/) is in Java 
...
- **BUT** recent NLP packages for R are `Rcpp::`-based extensions, and thus can overcome some of R's limitations (especially if you are using a high-performance machine and/or parallelization tricks)
