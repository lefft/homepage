...
dat <- dat[, c("source","created","date","hour","day","price")]
...
# classify here
waow_clf1 <- ml_classify(text=waow$text, id="cl_qkjxv9Ly")


waow_clf1 %>% group_by(label) %>% summarize(
  num=length(label),
  avg_prob=mean(probability, na.rm=TRUE)
)

table(waow_clf1$label)

# may want to remove some
barplot(table(waow_clf1$label))
hist(waow_clf1$probability)
...
# classify here
waow_clf2 <- ml_classify(text=waow$text, id="cl_RWBfoo2B")


waow_clf2 %>% group_by(label) %>% summarize(
  num=length(label),
  avg_prob=mean(probability, na.rm=TRUE)
)

table(waow_clf2$label)

# may want to remove some
barplot(table(waow_clf2$label))
hist(waow_clf2$probability)

```


```{r hashclf, fig.show='hold', fig.width=5, fig.height=3.5, out.width="47.5%"}
waow_hash <- twListToDF(sapply(dates, get_tweets, term="#bitcoin"))
# waow_hash2 <- twListToDF(sapply(dates, get_tweets, term="#btc"))

waow_hashclf <- ml_classify(text=waow_hash$text, id="cl_RWBfoo2B")

waow_hashclf %>% group_by(label) %>% summarize(
  num=length(label),
  avg_prob=mean(probability, na.rm=TRUE)
)

table(waow_hashclf$label)

# may want to remove some
barplot(table(waow_hashclf$label))
hist(waow_hashclf$probability)
```
